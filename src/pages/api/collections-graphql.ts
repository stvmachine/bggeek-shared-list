...
export default async function handler(
  ...
  try {
    console.log(`Fetching multiple collections via GraphQL for usernames: ${usernameArray.join(', ')}`);
    ...
    console.log(`GraphQL collections data received:`, {
      ...
      totalItems: validCollections.reduce((sum: number, col: any) => sum + (col.totalItems || 0), 0),
    });
    ...
    const transformedData = validCollections.map((collection: any) => ({
      ...
      item: collection.items?.map((item: any) => ({
    ...
    res.status(500).json({ error: "Failed to fetch collections data via GraphQL" });
  }
}
