...
import { fetchCollectionsGraphQL, mergeCollectionsGraphQL } from "../../api/fetchGroupCollectionGraphQL";
...
export default async function handler(
  ...
  const usernameArray = usernames.split(',').map(name => name.trim()).filter(Boolean);
  ...
  try {
    console.log(`Fetching group collection via GraphQL for usernames: ${usernameArray.join(', ')}`);
    ...
    console.log(`GraphQL group collection data received:`, {
      ...
      totalItems: mergedData.collections.reduce((sum, col) => sum + (col.totalitems || 0), 0),
    });
    ...
    res.status(500).json({ error: "Failed to fetch group collection data via GraphQL" });
  }
}
